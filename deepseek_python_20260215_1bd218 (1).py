...
def get_deals():
    try:
        ...
        user_id = get_user_id(telegram_id_int)
        if not user_id:
            return jsonify({"success": False, "error": "User not registered"}), 404

        # Query deals where user is buyer (advertiser), join campaigns and channels
        deals_resp = supabase.table("deals").select("*, campaigns(title), channels(username)").eq("buyer_id", user_id).execute()
        deals_data = deals_resp.data if deals_resp.data else []
        ...
        for deal in deals_data:
            ...
            allowed_transitions = DealStateMachine.get_allowed_transitions(status)
            ...
            enriched_deal = {
                ...
                'title': campaign.get('title') or f"Deal #{deal['id']}",
                'channel': channel.get('username') or '',
                'status': status,
                'label': label,
                'step': step,
                ...
                'escrow_amount': deal.get('amount', 0),
                'created_at': deal.get('created_at'),
                'type': 'advertiser',  # since we queried buyer deals
                # Include media fields if present (from campaign)
                'media_url': deal.get('media_url'),
        ...
        logger.error(f"Error in /api/deals: {e}")
