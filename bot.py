...
class DealStateMachine:
    ...
    # Transitions allowed via UI (user actions)
    # System transitions (funded, posted, verified, completed) are handled by backend logic
    UI_TRANSITIONS = {
        'pending': ['accepted', 'cancelled'],
        'accepted': ['cancelled'],  # 'funded' is done via payment
        'funded': ['scheduled'],    # 'posted' is done via auto-poster
        'scheduled': ['cancelled'],
        'posted': [],
        'verified': [],
        'completed': [],
...
class AdEscrowBot:
    ...
    def _load_mock_channels(self):
        ...
        try:
            with get_db() as conn:
                ...
                if count == 0:
                    logger.info("Database empty, loading mock channels...")
                    # Ensure a mock user exists for these channels
                    cursor.execute("INSERT OR IGNORE INTO users (telegram_id, role) VALUES (0, 'admin')")
                    mock_user_id = 0 # In a real scenario we'd get the ID, but let's assume 0 for internal
                    
                    # Actually we need a real ID potentially.
                    # Let's check or create a system user
                    cursor.execute("SELECT id FROM users WHERE telegram_id = 0")
                    sys_user = cursor.fetchone()
                    if not sys_user:
                        cursor.execute("INSERT INTO users (telegram_id, role) VALUES (0, 'system')")
                        sys_user_id = cursor.lastrowid
                    else:
                        sys_user_id = sys_user[0]

                    for ch_data in get_mock_channels():
                        cursor.execute('''
                            INSERT INTO channels (owner_id, username, name, category, subscribers, avg_views, price)
                            VALUES (?, ?, ?, ?, ?, ?, ?)
                        ''', (
                            sys_user_id,
                            ch_data["channel_handle"],
                            ch_data["channel_name"],
                            ch_data["category"],
                            ch_data["subscribers"],
                            ch_data["avg_views"],
                            ch_data["price_per_post"]
                        ))
                    conn.commit()
                    logger.info(f"Loaded {len(get_mock_channels())} mock channels into DB")
                else:
                    logger.info(f"Database already has {count} channels, skipping mock load")
        except Exception as e:
            logger.error(f"Error loading mock channels: {e}")
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Use a safer method for disable_web_page_preview to support both v13 and v20+
        # In v20+, it's link_preview_options, but disable_web_page_preview creates a warning or error.
        # We'll just omit it if it causes issues, or use kwargs safely.
        try:
            ...
            # Fallback for v20+ if disable_web_page_preview is removed
            await update.message.reply_text(
                text,
                reply_markup=reply_markup,
                parse_mode="Markdown"
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        Checks for WEBAPP_URL and sends welcome message with Mini App button.
        """
        ...
        if not webapp_url:
            await update.message.reply_text(
                "⚠ Mini App is not configured. Please contact admin."
            )
            return
    ...
    async def help_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Show full help guide via callback"""
    ...
    async def back_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Return to start screen via callback"""
        query = update.callback_query
        await query.answer()
...
def api_get_deals():
    ...
    try:
        with get_db() as conn:
            ...
            # Enrich with state machine info for Frontend
            for d in deals:
                d['step'] = DealStateMachine.get_step(d['status'])
                d['label'] = DealStateMachine.get_label(d['status'])
                d['is_terminal'] = DealStateMachine.is_terminal(d['status'])
                # Add allowed UI transitions only
                d['allowed_transitions'] = DealStateMachine.UI_TRANSITIONS.get(d['status'], [])
...
def api_transition_deal(deal_id):
    ...
    try:
        ...
        deal_info = get_deal_with_state_info(deal_id)
        if not deal_info:
            return jsonify({'success': False, 'error': 'Deal not found'}), 404
        
        current_state = deal_info['status']
        
        # Restrict API transitions to UI_TRANSITIONS for security
        allowed_ui_transitions = DealStateMachine.UI_TRANSITIONS.get(current_state, [])
        
        if new_state not in allowed_ui_transitions:
            return jsonify({
                'success': False,
                'error': f"Invalid UI transition: '{current_state}' to '{new_state}'. Allowed: {allowed_ui_transitions}",
                'current_status': current_state,
                'allowed_transitions': allowed_ui_transitions
            }), 400
        
        # Perform transition using the internal state machine (which has full transitions)
        # The check above ensures only UI-allowed transitions reach here.
