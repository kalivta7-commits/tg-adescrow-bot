...
def api_accept_deal(deal_id):
    try:
        ...
        # Use transition_deal_state for consistent logic & notifications
        # (It doesn't strictly check channel permissions internally, but we can rely on 
        #  the state machine or add explicit checks if needed. 
        #  For now, consistent with user request to use transition_deal_state)
        
        # However, to be safe, we should probably check permissions here too or 
        # ensure transition_deal_state handles it? 
        # The user said "Call transition_deal_state". 
        # I'll stick to that, but note that the explicit routes usually imply permission checks.
        # Let's add permission checks back since I removed them.
...
def main():
    ...
    # Database initialized via Supabase
