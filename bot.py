...
def api_get_deals():
    try:
        ...
        # Helper to process deals
        def process_deals(deals_data, role_type):
            processed = []
            for d in deals_data:
                d['type'] = role_type
                # Map joined fields to flat fields for frontend
                # channels(username) -> d['channel']
                if d.get('channels'):
                    # Handle if returns list or dict
                    ch = d['channels']
                    if isinstance(ch, list) and ch:
                        d['channel'] = ch[0].get('username')
                    elif isinstance(ch, dict):
                        d['channel'] = ch.get('username')
                
                # campaigns(title) -> d['title']
                if d.get('campaigns'):
                    cmp = d['campaigns']
                    if isinstance(cmp, list) and cmp:
                        d['title'] = cmp[0].get('title')
                    elif isinstance(cmp, dict):
                        d['title'] = cmp.get('title')
                processed.append(d)
            return processed

        # 1. Fetch deals where user is the BUYER (Advertiser)
        # Try to join with channels and campaigns for better UI data
        # Note: If FKs are missing, this might return null for joined fields, which is handled.
        buyer_query = supabase.table("deals").select("*, channels(username), campaigns(title)")
        
        buyer_deals_resp = (
            buyer_query
            .eq("buyer_id", user_id)
            .in_("status", ["waiting_payment", "paid", "ad_posted", "completed", "disputed"])
            .execute()
        )
        buyer_deals = process_deals(buyer_deals_resp.data or [], 'campaign')
        
        # 2. Fetch deals where user is the SELLER (Channel Owner)
        # First get channels owned by user from Supabase channels table
        channels_resp = (
            supabase.table("channels")
            .select("id")
            .eq("owner_id", user_id)
        ...
        owned_channel_ids = [c['id'] for c in (channels_resp.data or [])]
        
        seller_deals = []
        if owned_channel_ids:
            seller_deals_resp = (
                supabase.table("deals")
                .select("*, channels(username), campaigns(title)")
                .in_("channel_id", owned_channel_ids)
                .in_("status", ["waiting_payment", "paid", "ad_posted", "completed", "disputed"])
                .execute()
            )
            seller_deals = process_deals(seller_deals_resp.data or [], 'placement')

        # Merge results (Buyer deals priority if duplicate)
        all_deals = {}
        for d in buyer_deals:
            all_deals[d['id']] = d
        for d in seller_deals:
            if d['id'] not in all_deals:
                all_deals[d['id']] = d
            
        final_deals = list(all_deals.values())
        
        # Sort by created_at descending (newest first)
        final_deals.sort(key=lambda x: x.get('created_at', ''), reverse=True)

        return json_response(True, data={'deals': final_deals})

    except Exception as e:
        logger.error(f"Error getting deals: {e}", exc_info=True)
        # Fallback to simple query if joins fail due to missing FKs
        try:
            logger.info("Retrying deals fetch without joins...")
            fallback_resp = (
                supabase.table("deals")
                .select("*")
                .eq("buyer_id", user_id)
            ...
            deals = fallback_resp.data or []
            for d in deals: d['type'] = 'campaign' # Default to campaign
